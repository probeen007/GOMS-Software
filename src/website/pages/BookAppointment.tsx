...
import { CheckCircle, CalendarCheck, UserCheck, ShieldCheck, Sparkles, Timer } from 'lucide-react'
  ...
  if (submitted) {
    return (
      <div>
        ...
        />
        <section style={{ background: '#FFFFFF', padding: '120px 0', minHeight: '60vh' }}>
          <div className="pm-container">
            ...
            >
              ...
              <h2 style={{ fontFamily: 'Barlow Condensed, sans-serif', fontSize: '2.5rem', fontWeight: 800, textTransform: 'uppercase', color: '#0F172A', marginBottom: '16px' }}>
                Appointment Requested!
              </h2>
              <p style={{ fontSize: '15px', color: '#475569', lineHeight: 1.7, marginBottom: '32px' }}>
                Thank you, <strong style={{ color: '#0F172A' }}>{form.fullName}</strong>! Your appointment request has been received. Our team will contact you within 2 hours to confirm.
              </p>
      ...
      <section style={{ background: '#F8FAFC', padding: '80px 0 120px' }}>
        <div className="pm-container">
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-12">
            ...
            >
              <div style={{ background: '#FFFFFF', border: '1px solid #E2E8F0', borderRadius: '12px', padding: '48px', boxShadow: '0 4px 20px rgba(0,0,0,0.03)' }}>
                <div className="section-label">Book Online</div>
                <h2 className="heading-section text-slate-900 mb-3">Appointment Details</h2>
                <div className="red-line" style={{ marginBottom: '36px' }} />

                {error && (
                  <div style={{ background: '#FEF2F2', border: '1px solid #FCA5A5', color: '#991B1B', padding: '12px 16px', borderRadius: '6px', fontSize: '13px', marginBottom: '24px' }}>
                    {error}
                  </div>
                )}
                ...
                <form onSubmit={handleSubmit}>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-5 mb-6">
                    ...
                    <FormField label="Phone Number *">
                      <input type="tel" name="phone" value={form.phone} onChange={handleChange} placeholder="01-4525461 or 98X-XXX-XXXX" required className="pm-input" />
                    </FormField>
                  ...
                  <div className="grid grid-cols-1 md:grid-cols-2 gap-5 mb-8">
                    ...
                    <FormField label="Preferred Date *">
                      <input type="date" name="preferredDate" value={form.preferredDate} onChange={handleChange} required className="pm-input" min={new Date().toISOString().split('T')[0]} style={{ colorScheme: 'light' }} />
                    </FormField>
            ...
            <motion.div initial={{ opacity: 0, x: 24 }} animate={{ opacity: 1, x: 0 }} transition={{ duration: 0.7, delay: 0.15 }} className="flex flex-col gap-6">
              <div style={{ background: '#FFFFFF', border: '1px solid #E2E8F0', borderRadius: '12px', padding: '32px', boxShadow: '0 4px 15px rgba(0,0,0,0.03)' }}>
                <div style={{ fontFamily: 'Barlow Condensed, sans-serif', fontSize: '1.3rem', fontWeight: 800, textTransform: 'uppercase', color: '#0F172A', marginBottom: '20px', letterSpacing: '0.05em' }}>
                  Appointment Info
                </div>
                <div className="space-y-5">
                  {[
                    { icon: CalendarCheck, title: 'Flexible Scheduling', desc: 'Choose a date and time that works for you.' },
                    { icon: UserCheck, title: 'Expert Technicians', desc: 'All work by certified professionals.' },
                    { icon: ShieldCheck, title: 'Quality Service', desc: '3-month warranty on repairs and parts.' },
                    { icon: Sparkles, title: 'Customer Satisfaction', desc: '100% satisfaction guarantee.' },
                    { icon: Timer, title: 'Quick Turnaround', desc: 'Efficient service, minimal wait times.' },
                  ].map(({ icon: Icon, title, desc }, i) => (
                    <div key={i} className="flex items-start gap-4">
                      <div style={{ width: '36px', height: '36px', background: 'rgba(230,57,70,0.1)', borderRadius: '8px', display: 'flex', alignItems: 'center', justifyContent: 'center', flexShrink: 0 }}>
                        <Icon size={18} color="#E63946" />
                      </div>
                      <div>
                        <div style={{ fontSize: '13px', fontWeight: 800, color: '#0F172A', marginBottom: '3px' }}>{title}</div>
              ...
              <div className="shadow-md" style={{ borderRadius: '12px', overflow: 'hidden', aspectRatio: '4/3' }}>
                <img
...
function SectionDivider({ label }: { label: string }) {
  return (
    <div style={{ fontSize: '12px', fontWeight: 800, letterSpacing: '0.15em', textTransform: 'uppercase', color: '#64748B', marginBottom: '16px', paddingTop: '8px' }}>
      {label}
    </div>
  )
}
...
function FormField({ label, children, className = '' }: { label: string; children: React.ReactNode; className?: string }) {
  return (
    <div className={className}>
      <label style={{ display: 'block', fontSize: '12px', fontWeight: 700, color: '#475569', marginBottom: '8px', letterSpacing: '0.05em', textTransform: 'uppercase' }}>
        {label}
      </label>
      {children}
    </div>
